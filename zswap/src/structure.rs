...
#[storable(base)]
#[tag = "zswap-split-public-inputs[v3]"]
pub struct SplitPublicInputs {
    pub public_key: CoinPublicKey,
    pub coin_commitment: Commitment,
    pub coin_binding_tag: Fr,
    /// v3: Poseidon commitment `C_sk = transientHash("midnight:sk-commit[v1]", sk, r)`
    /// disclosed by both the wallet attestation and the per-spend proof. The
    /// admission verifier cross-checks these for byte-equality; Poseidon
    /// binding under collision resistance then forces both to have opened
    /// the same 32-byte `sk`.
...
pub struct SplitProofBundle {
    ...
    /// v3: one-time-per-wallet attestation proof. Generated by
    /// `circuits/wallet_attestation.compact`; carried in every split-send
    /// bundle and re-verified by the node admission code. Binds the canonical
    /// `pk = persistentHash(sep, sk)` to the Poseidon `commitment_sk`.
...
// v3 magic — bumped from `:v2` so old (no-attestation) bundles fail closed
// against the v3 node verifier.
const SPLIT_PROOF_BUNDLE_MAGIC: &[u8] = b"midnight:zswap-split-proof-bundle:v3";
...
impl SplitProofBundle {
    ...
    ///   u32 LE len ‖ attestation_proof bytes ‖
    ///   public_key[32] ‖ coin_commitment[32] ‖ coin_binding_tag[32] ‖ commitment_sk[32]
    pub fn encode(self) -> Proof {
        ...
        append_len_prefixed(&mut bytes, &self.attestation_proof.0);
        ...
        bytes.extend_from_slice(&self.split_public_inputs.commitment_sk.as_le_bytes());
        Proof(bytes)
    }
...
pub struct SplitInput<D: DB> {
    ...
    pub split_public_inputs: SplitPublicInputs,
    pub client_derivation_proof: Proof,
    /// v3: per-wallet attestation proof, passed through verbatim to the
    /// emitted bundle. Generated once at wallet setup via
